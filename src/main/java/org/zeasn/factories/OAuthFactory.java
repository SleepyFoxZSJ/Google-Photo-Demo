...
import org.zeasn.exceptions.IdTokenException;
...
public class OAuthFactory {
    ...
    /**
     * 通过idToken获取userId
     * @param idToken 客户端授权获取
     * @return 用户Id
     */
    public static String getUserId(String idToken) throws GeneralSecurityException, IOException, IdTokenException {
        GoogleIdTokenVerifier idTokenVerifier = new GoogleIdTokenVerifier.Builder(GoogleNetHttpTransport.newTrustedTransport(), JSON_FACTORY)
                .setAudience(Collections.singleton(Constants.CLIENT_ID))
                .build();
        GoogleIdToken idTokenInfo = idTokenVerifier.verify(idToken);
        if (idTokenInfo != null) {
            GoogleIdToken.Payload payload = idTokenInfo.getPayload();
